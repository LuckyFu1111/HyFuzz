#!/usr/bin/env python3
"""
Modbus/TCP Validity and Exception Profile Tests
Tests for PSR (Protocol Success Rate) vs EXR (Exception Rate) analysis
"""
...
from protocols.protocol_handler import ProtocolSessionState


class ModbusValidityTester:
    """Test Modbus validity metrics"""
    ...
    async def test_validity_profiles(self, num_trials: int = 1000) -> Dict:
        ...
        Returns PSR (Protocol Success Rate) and EXR (Exception Rate) data
        """
        results = {
            'total_requests': 0,
        ...
        for trial in range(num_trials):
            ...
            # Initialize function code stats if needed
    ...
    async def test_state_progress(self, num_trials: int = 500) -> Dict:
        ...
        Tracks FC × address × unit-id coverage
        """
        state = ProtocolSessionState()
        state.session_id = "validity_test"

        coverage = {
            'fc_address_coverage': set(),  # (fc, address_bin)
            'unique_states': 0,
            'first_hit_times': {},
            'state_transitions': []
        }
        ...
        address_bins = list(range(0, 65536, 1000))  # Bin addresses by 1000s

        print(f"Running {num_trials} stateful Modbus tests...")

        for trial in range(num_trials):
            fc = function_codes[trial % len(function_codes)]
            addr = (trial * 137) % 65536  # Pseudo-random walk
            ...
            response = await self.handler.execute_stateful(params, state)
