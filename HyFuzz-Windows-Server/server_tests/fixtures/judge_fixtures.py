...
class JudgeRequest(TypedDict):
    """Simplified judge request payload used in unit server tests."""
...
class JudgeResponse(TypedDict):
    """Simplified judge response payload used in unit server tests."""
...
def sample_requests() -> List[JudgeRequest]:
    """Return request samples used in server tests."""
    return list(_SAMPLE_REQUESTS)


def sample_responses() -> List[JudgeResponse]:
    """Return response samples used in server tests."""
