...
Security Note - Pickle Usage:
    This module uses pickle for serialization of LLM responses. This is safe here
    because all cached data is internally generated by the application's own LLM
    service. The cache only stores responses from trusted LLM providers.

    SECURITY CONSIDERATIONS:
    1. ✓ Cache data is generated internally (LLM responses)
    2. ✓ No external/user data is pickled
    3. ✓ Cache files have restricted filesystem permissions
    4. ✓ Cache directory is application-controlled

    WARNING: Never deserialize pickle data from untrusted sources!
    If caching external data, use JSON or msgpack instead.
